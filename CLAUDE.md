...
1. **MCP credentials must be updated per customer.** When the user asks for a new PoC subsite (or any verification work in DevRev), the first action — *before writing code* — is to confirm that the `user-devrev` MCP credentials point to the new customer's DevRev workspace. There is a single `user-devrev` server (the user updates its credentials manually when switching customers); historically there were per-customer `user-devrev_<Customer>` servers but that scheme is deprecated. Confirm by calling `get_current_user` and matching the `devoid` against the workspace that owns the customer's plug `app_id`. Without this, any DevRev-side verification produces data from the wrong tenant.
...
7. **Logout must hard-reload the page.** `plugSDK.shutdown()` does NOT remove the launcher / widget iframes it injected, nor invalidate the active DevRev session token. If `logout()` only clears `localStorage` and calls `applyAuthState(null)`, the old iframes stay alive in the DOM with the previous user's identity. A subsequent re-login as a different user causes `plugSDK.init({newIdentity})` to inject a *second* set of iframes alongside the old ones, so two plug sessions run in parallel — both fire `rev-users.identify`, and DevRev attributes new conversations / tickets to whichever iframe handles the user's click (typically the older one). Visible symptom: every chat in DevRev appears under the first user who ever opened the chatbot in that browser session. Fix that every subsite **must** carry: `logout()` ends with `location.reload()` (see commit `b917c26`). Do not "optimise" this away to keep logout snappy — the runtime identity-switch path in plugSDK is broken and a reload is the only reliable cure.
